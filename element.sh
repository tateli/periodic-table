  ...
  then
    ...
    if [[ -z $ELEMENT ]]
      then
        ...
        echo $ELEMENT | while IFS=\| read AN AM MPC BPC SY NAME TYPE
        do
          echo "The element with atomic number $AN is $NAME ($SY). It's a $TYPE, with a mass of $AM amu. $NAME has a melting point of $MPC celsius and a boiling point of $BPC celsius." 
        done
    fi
  else
